...
- ✅ The customer journey is bilingual end to end: a language gate on first visit, then landing, signup, dashboard, editor, billing, support, the public page and every email in that language — including the starter portfolio content a new account is given. The owner's own prose (terms, privacy, rules, support hours, announcements) has an optional English pair that falls back to the Arabic. **The console is partly done** — shell, nav and overview; the remaining console pages are still Arabic-only.
