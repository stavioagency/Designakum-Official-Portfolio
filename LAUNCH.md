...
- ✅ The customer journey is bilingual end to end: a language gate on first visit, then landing, signup, dashboard, editor, billing, support, the public page and every email in that language — including the starter portfolio content a new account is given. The owner's own prose (terms, privacy, rules, support hours, announcements) has an optional English pair that falls back to the Arabic. The console is bilingual too, with its own language switch, and a test asserts no Arabic interface text survives on any console page in English — customer names, report text and the owner's own policies stay in whatever language they were written in, which is the point. Server actions answer in the caller's language too — they used to reply in Arabic regardless, which only showed after someone pressed save.
